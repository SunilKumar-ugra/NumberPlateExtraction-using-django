...
classifications_path = os.getcwd() + '\\NumberPlate' +'\\classifications.txt'
...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt('NumberPlate_App\classifications.txt', np.float32)                  # read in training classifications
    ...
    try:
        ...
        npaFlattenedImages = np.loadtxt("NumberPlate_App\\flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    contours,npahierachy = cv2.findContours(imgThreshCopy,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
