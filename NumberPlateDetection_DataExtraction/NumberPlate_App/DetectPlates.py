...
from NumberPlate_App import Preprocess
from NumberPlate_App import DetectChars
from NumberPlate_App import PossiblePlate
from NumberPlate_App import PossibleChar
...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main1.showSteps == True: # show steps #######################################################
        ...
        for i in range(0, len(listOfPossiblePlates)):
            ...
            cv2.line(imgContours, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), Main1.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), Main1.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), Main1.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), Main1.SCALAR_RED, 2)
...
def findPossibleCharsInScene(imgThresh):
    ...
    contours,npahierachy=cv2.findContours(imgThreshCopy,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)  # find all contours
